    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8">
        <form method="POST" action="{{ route('periods.update', $period) }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="number" name="due_amount" id="due_amount" value="{{ old('due_amount', $period->due_amount ? (int) $period->due_amount : '') }}" min="0" step="1" placeholder="Kosongkan jika mengikuti default grup"
            ...
            <div>
                <label for="status" class="block text-xs font-semibold text-slate-700 mb-1.5">Status Periode <span class="text-red-500">*</span></label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 shadow-xs transition-colors">
                    <i class="bi bi-check-lg"></i> Perbarui Periode
                </button>
                <a href="{{ route('periods.index', ['group_id' => $period->group_id]) }}" class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
                    Batal
                </a>
            </div>
        </form>
